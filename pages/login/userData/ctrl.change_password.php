...
if (isset($_POST['submit']) && !empty($_SESSION['username']) && !empty($_SESSION['code'])) {
    ...
    if ($password != $con_password) {
        ...
        $board_memb = $db->query("SELECT * FROM tbl_board_memb WHERE username = '$username'");
        $numrow9 = $board_memb->num_rows;
        ...
        if ($numrow > 0) {
            ...
            $query1 = $db->query("UPDATE tbl_board_memb SET password = '$hashed_password' WHERE username = '$username'");
        }
