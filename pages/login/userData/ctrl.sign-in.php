...
if (isset($_POST['signin']) || isset($_SESSION['confirm'])) {
    ...
    $board_memb = mysqli_query($db, "SELECT * FROM tbl_board_memb WHERE username = '$username'");
    $numrow9 = mysqli_num_rows($board_memb);
    ...
    if ($numrow > 0) {
        ...
        while ($row = mysqli_fetch_array($board_memb)) {
            $hashedPwdCheck = password_verify($password, $row['password']);
            if (false == $hashedPwdCheck) {
                $_SESSION['sessionP'] = true;
                header("location: ../sign-in.php");
                ...
                $_SESSION['role']   = "Board Member";
                $_SESSION['userid'] = $row['board_memb_id'];
                $_SESSION['name']   = $row['lastname'] . ", " . $row['firstname'];
            }
            header("location: ../../dashboard/index.php");
        }
